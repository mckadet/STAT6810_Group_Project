...
spplot(cad.raw['var1.pred'], 
       main = "Predicted Cd Concentration (inverse distance)")
...
ggplot(as.data.frame(jura.pred), 
       aes(x = Zn, 
           y = Cd)) + 
...
# add data
jura.g <- gstat(NULL, id = "Cd", 
                formula = log(Cd) ~ 1, 
                data = jura.pred, 
                nmax = 10)
jura.g <- gstat(jura.g, id = "Zn", 
                formula = log(Zn) ~ 1, 
                data = jura.pred, 
                nmax = 10)
...
pl1 <- spplot(cz["Cd.pred"], 
              main = "log-cadmium prediction")
pl2 <- spplot(cz["Zn.pred"], 
              main = "log-zinc prediction")
pl3 <- spplot(cz["Cd.se"], 
              main = "log-cadmium standard error")
pl4 <- spplot(cz["Zn.se"], 
              main = "log-zinc standard error")
